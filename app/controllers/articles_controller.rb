class ArticlesController < ApplicationController
	...
	#除了 index 和 show action，其他动作都要通过身份验证才能访问
	...
	def create
		...
		if @article.save #若save成功
			...
			render 'new' #使用 render 方法是为了把 @article 对象回传给 new 模板。
			#这里渲染操作是在提交表单的这个请求中完成的，而 redirect_to 会告诉浏览器发起另一个请求。
		end
	end
	
	def update #update Action用于更新已有记录，它接受一个散列hash作为参数，散列中包含想要更新的属性。
		@article = Article.find(params[:id])

		if @article.update(article_params)
			redirect_to @article  #相当于：redirect_to article_path(@article) 
		else
			render 'edit'  #render edit.html.erb  如果更新文章时发生错误，就需要把表单再次显示给用户。
		end
	end
	...
	def destroy
		...
		redirect_to articles_path #articles_path: localhost:3000/articles
	end
